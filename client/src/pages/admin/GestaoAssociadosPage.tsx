 ...
 * Hora: 16:30
 * Versão: 1.2 (Adiciona Exclusão Permanente e Corrige Handler)
 * Tarefa: 277
 *
 * Descrição: Página dedicada à Gestão de Associados.
 * CORRIGIDO: O handler de aprovação/rejeição foi corrigido para enviar
 * o token no body (Plano G). Adicionado o botão TRASH (Exclusão Permanente).
 *
 * ==========================================================
 */
...
export default function GestaoAssociadosPage() {
  ...
  // 2. CORREÇÃO: Handler para Aprovar/Rejeitar (Apenas muda o Status)
  const handleAtualizarAssociado = async (idAssociado: number, novoStatus: 'aprovado' | 'rejeitado') => {
    if (!token) return;
    try {
      // CORREÇÃO DE PAYLOAD: {token, data: {...}}
  ...
  const handleApagarAssociado = async (idAssociado: number, nomeAssociado: string) => {
    ...
    try {
      const payload = { token: token, data: { id_atleta: idAssociado }}; // CORREÇÃO DE PAYLOAD: {token, data: {...}}
