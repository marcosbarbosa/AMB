// Nome: DiretoriaPremium.tsx
// Caminho: client/src/components/DiretoriaPremium.tsx
// Data: 2026-01-19
// Hora: 22:15
// Função: Galeria da Diretoria (Path Fix & Cache Busting)
// Versão: v7.0 Path Sync
// Alteração: Sincronização com a pasta /uploads/diretoria e fix de cache.
...
// CORREÇÃO CRÍTICA: Apontar para a mesma pasta onde o Admin salva
...
export function DiretoriaPremium() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Timestamp para evitar cache de JSON antigo
  ...
  const getImageUrl = (dbUrl: string | null) => {
    ...
    // 2. Se for URL absoluta (http...), usa direto
    if (cleanUrl.startsWith('http')) return cleanUrl;

    // 3. Se for caminho relativo antigo (começando com ../ ou assets)
    // Extrai apenas o nome do arquivo para usar com o novo BASE_URL
    if (cleanUrl.includes('/')) {
        cleanUrl = cleanUrl.split('/').pop() || cleanUrl;
    }

    // 4. Monta a URL correta + Cache Buster
    return `${BASE_IMG_URL}${cleanUrl}?t=${new Date().getTime()}`; // O timestamp força a atualização visual
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-slate-600 text-lg leading-relaxed">
            Profissionais dedicados que trabalham incansavelmente para fortalecer o basquete master no Amazonas.
          </p>
        </div>
                             ...
                             <img 
                               ...
                               onError={(e) => {
                                   ...
                                   if(parent) {
                                       ...
                                       parent.innerHTML = '<div class="flex items-center justify-center h-full w-full"><svg class="w-12 h-12 text-slate-300" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div>';
                                   }
                               }}
                             />
                           ) : (
...
// linha 155 DiretoriaPremium.tsx